...
font.y = 0.95
font.cex = 0.8
pdf("scoring_metric_data/sim_plots/all_sim2.pdf",height=7,width=7.1)
grid.newpage()
...
useful.prop <- plot.sim(sims,c(NA,0,NA,0),limits=TRUE,metrics =c("Pearson","Pseudo.V","MCC","AUPR","X2A_metric","SCC","SSE"), lab=c("AUPR","Pearson/MCC","JS divergence","SCC", "SSE","2A score"), filename="scoring_metric_data/sim_plots/useful_prop_all.png",param.interest="K.u",key.x=5,key.y=-0.5)
...
cairo_pdf("scoring_metric_data/sim_plots/all_metrics_all_sim2.pdf",height=7,width=7.1)
grid.newpage()
lay1 <- grid.layout(2,2,heights=unit(c(3.5,3.5),c("in","in")),widths=unit(c(3.5,3.5),c("in","in")))
vp1 <- viewport(layout=lay1,name="top.vp",height=unit(7,"in"),width=unit(7,"in"))
pushViewport(vp1)
